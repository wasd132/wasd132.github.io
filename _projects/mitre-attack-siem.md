...
summary: "제로데이 역직렬화 공격에 대해 Falco(커널 시스템 콜 감시) + Wazuh(SIEM) + Shuffle(SOAR)를 결합하여, 탐지부터 격리까지 10초 이내 자동 대응을 실현한 차세대 보안 운영 아키텍처입니다."
...
기존 경계 보안 솔루션(FW, IPS, WAF)은 시그니처 기반으로 동작하여 제로데이 역직렬화(Deserialization) 공격 페이로드를 정상 트래픽으로 오인합니다. OWASP Top 10:2021 "Software and Data Integrity Failures"와 CWE-502 위협이 증가하는 가운데, **"Assume Breach"** 전제 하에 내부 프로세스 레벨에서 이상 징후를 식별하고 자동 대응하는 차세대 보안 운영 모델의 필요성이 대두되었습니다.

이를 위해 팀 Interrupt(4명)에서 Falco(커널 시스템 콜 감시) + Wazuh(SIEM) + Shuffle(SOAR) 기반의 통합 탐지·대응 시스템을 구축하여, **탐지부터 격리까지 10초 이내 자동 대응**을 실현했습니다.
...
5. **SCA 위험도 반전**: $RiskScore = 100 - SCA\ Score$ (취약한 서버일수록 가중치 ↑)
6. **가중 스코어링**: 행위별 차등 점수 (랜섬웨어 +20, RCE/리버스쉘 +15, 권한 상승 +12 등)
7. **등급 판정·대응 실행**
...
```
외부 공격자 → OPNsense(FW) → WAF → Docker 컨테이너(Next.js/Java/PHP)
                                            ↓
                                    Falco (커널 시스템 콜 감시)
                                    Wazuh Agent (FIM/SCA/로그 수집)
                                            ↓
                                    Wazuh Manager (정규화·상관분석)
                                            ↓ (Webhook)
                                    Shuffle (SOAR)
                                    ├── AbuseIPDB (IP 평판)
                                    ├── MISP (TI, 해시 대조)
...
| 6. 지속성 | API Route에 eval() 백도어 삽입 | TA0003(T1505.003) | 탐지 |
...
- 공격 성공 후 PID 1(Java 프로세스)에서 `/bin/sh`가 비정상적으로 Fork되는 패턴을 확인하고, 이를 Falco 커스텀 룰의 탐지 기준으로 제안했습니다. 이 발견이 "언어별 진입점은 상이하나 공격 성공 후 시스템 행위는 수렴한다"는 핵심 결론을 도출하는 근거가 되었습니다.
- Java Gadget Chain(readObject() 메서드 악용)의 동작 원리를 분석하고, PHP의 매직 메서드(__wakeup, __destruct), Node.js의 Prototype Pollution과 비교하여 다중 언어 공통 탐지 정책 수립에 기여했습니다.
...
**해결**: 6,000+ 앱 에코시스템과 No-Code 워크플로우를 제공하는 Shuffle로 전환하여, Wazuh 웹훅 → 스코어링 → 자동 대응까지의 파이프라인을 유연하게 구축했습니다.
