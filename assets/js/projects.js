...
$(document).ready(() => {
    render_projects_split();
    // 버튼 클릭 시에도 2컬럼 분할 렌더링 유지
    $('.white-button').on('click', function() {
        render_projects_split();
    });
});
// 2컬럼 분할 렌더링 함수
let render_projects_split = () => {
    let security_projects = projects_obj.filter(p => p.categories.includes('security')).map(project_mapper).join('');
    let ml_projects = projects_obj.filter(p => p.categories.includes('ml')).map(project_mapper).join('');
    $(".projects-security-list").html(security_projects);
    $(".projects-ml-list").html(ml_projects);
}
